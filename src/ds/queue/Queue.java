...
public class Queue {
    ...
    public void insert(long j) { //we have a parameter that accepts data types of long because our array also accepts long
        ...
        } //when we insert a new element in the main, the new number will override and replace whatever is in the first position. since
        ...
        rear++;//**Whenever we insert items into a queue, they go to the end of the array**. When we increment it goes from -1 to 0
    ...
    public long remove() {
       ...
       long temp = queArray[front]; //first get element in front of line. we extract whatever is in the index position of the "front" value
        front ++; //since we pulled the index value from the original front, we increment to create the new front position
    ...
    public long peekFront() {
        return queArray[front]; //return the queue array, at the position front. Front is a pointer, we are not changing the value, we are just observing who is at front
    }
